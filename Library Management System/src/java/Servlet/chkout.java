...
public class chkout extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			query2 = "SELECT n - (SELECT count(*) from book_loans AS bl WHERE bl.book_id =\'"
					+ bookid
					+ "\' AND bl.branch_id =\'"
					+ branchid
			...
			while (rs5.next()) {
				finesPaid = false;
                        ...
                        /*write.println("<html>");
				write.println("<body>123</body></html>");*/
			if (CheckOutMaxLimit && BookAvailable && SameBook && finesPaid) {
				rs3 = stmt3.executeUpdate(query3);
				Done = true;
			} else if (!finesPaid) {
				write.println("<html>");
				write.println("<body> <center><h2>Can't Check out Book As Borrower has not paid fines!</h2>");

				write.println("</center></body>");
				write.println("</html>");
			} else if (!SameBook) {
				write.println("<html>");
				write.println("<body><center> <h2>Can't Check out Same Book More Than Once!</h2>");
		...
		if (Done) {
			...
			write.println("<html>");
			write.println("<body><center> <h2>Check Out Successful !!</h2>");

			write.println("</center></body>");
			write.println("</html>");
		} else if (!CheckOutMaxLimit) {
			write.println("<html>");
			write.println("<body><center> <h2>Maximum Check Out Limit Reached !!</h2><h3> Cannot checkout more than 3 books</h3>");

			write.println("</center></body>");
			write.println("</html>");
		} else if (!BookAvailable) {
			write.println("<html>");
			write.println("<body><center> <h2>Book not available</h2>");
